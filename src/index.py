...
class Indexer(object):
    ...
    def __init__(self, vector_sz, n_subquantizers=0, n_bits=8, metric=None):
        assert n_subquantizers == 0
        assert metric in ['cosine_similarity', 'inner_product'], f'Unsupported metric {metric}'
    ...
    def search_knn(self, query_vectors: np.array, top_docs: int, index_batch_size: int = 2048) -> List[Tuple[List[object], List[float]]]:
        ...
        for k in tqdm(range(nbatch), desc='Searching index'):
...
class DataStore(object):
    def __init__(self,
        ...
        self.merge_database = merge_database
        self.merge_embed_paths = merge_embed_paths
        ...
        elif self.merge_database:
            assert self.merge_embed_paths is not None and self.merge_alphas is not None
            assert len(self.merge_embed_paths) == len(self.merge_alphas)
            num_emb_files = len(self.merge_embed_paths)
            num_samples_per_database = [int(dstore_size * self.merge_alphas[i]) for i in range(num_emb_files)]
            num_samples_per_database[-1] = dstore_size - sum(num_samples_per_database[:-1])
            self.embs = []
            for i in range(num_emb_files):
                emb = load_embeds(self.merge_embed_paths[i], num_samples_per_database[i], dimension, dtype)
                print(f"Loaded {num_samples_per_database[i]} samples from {i}-th database with shape {emb.shape}")
                self.embs.append(emb)
            self.embs = np.concatenate(self.embs, axis=0)
            print(f"Final loaded a hybrid database of shape {self.embs.shape}")
...
def get_index_dir_and_passage_paths(cfg, index_shard_ids=None):
    ...
    embedding_shard_save_path = embedding_args.embedding_dir + f"/passage_embeddings.pkl"
    embedding_paths = [embedding_shard_save_path]
...
def build_index(cfg):
    index_args = cfg.datastore.index
    if "bm25" in cfg.datastore.embedding.model_name_or_path.lower():
        return build_index_bm25(cfg)
    ...
    for index_shard_ids in index_shard_ids_list:
        # todo: support PQIVF
        index = Indexer(index_args.projection_size, index_args.n_subquantizers, index_args.n_bits, index_args.metric)
...
def get_index_passages_and_id_map(cfg, index_shard_ids=None):
    # todo: double check if the id map is correct when there's a 1-n mapping between index and passages
    index_args = cfg.datastore.index
    ...
    return passages, passage_id_map# Modified based on https://github.com/facebookresearch/contriever/blob/main/passage_retrieval.py
...
def get_bm25_index_data_dir(cfg):
    index_dir = os.path.join(cfg.datastore.embedding.embedding_dir, 'bm25_index')
    data_dir = os.path.join(cfg.datastore.embedding.embedding_dir, "bm25_data")
    os.makedirs(data_dir, exist_ok=True)
    return index_dir, data_dir

def build_index_bm25(cfg):
